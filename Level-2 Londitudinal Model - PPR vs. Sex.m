...
ytotal(:,j)= [y]            % store line function of each line of each regression in single matrix
    if mat(i,6)==0          % set predictor variable function in design matrix
     XTA(:,2)=0         
     end
XTA(:,3)=x/100               % set x value in design matrix
XTA(:,4)=(x.^2)/100         % set x squared value in design matrix
XTA(:,5)= ((XTA(:,2)).*(XTA(:,3)))/100 % set x value as a function of the predictor variable in design matrix
XTA(:,6)=((XTA(:,2)).*(XTA(:,4)))/100     % set x value squared as a function of the predictor variable in design matrix
...
[b,sig,E,V,loglikF] = mvregress(XT,ytotal');   %annalyze reggresion of design matrice array and y function matrix
[b sqrt(diag(V))]
...
title('Percentage of Pain Relief by Gender')
ylabel('PPR')
xlabel('Treatment')
hold off
